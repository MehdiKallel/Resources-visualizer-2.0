class ExpressionBuilder {
    ...
    highlightGraph() {
      ...
      this.currentExpression.forEach(blockOrItem => {
          const items = blockOrItem.type==="andBlock"
          ? blockOrItem.items
          : [ blockOrItem ];
        ...
        items.forEach(it=>{
          if (it.type==="subject") {
            ...
            document.querySelectorAll(`table.subject[data-uid="${it.value}"], g#${it.value}`)
              .forEach(el=>el.classList.add("expr-highlight"));
          }
          else if (it.type==="skill") {
             document.querySelectorAll(`path[id="${it.pathId}"]`)
              .forEach(el=>el.classList.add("expr-highlight"));
          }
            ...
            document.querySelectorAll(`text`).forEach(textElement => {
              if (textElement.textContent.trim() === it.value) {
                console.error("highlighting unit/role", textElement);
    ...
    handleSkillDrop({ detail }) {
      ...
      const last = this.currentExpression[this.currentExpression.length - 1];
    ...
    createExpressionBuilderUI() {
      ...
      expressionDisplay.addEventListener("drop", (e) => {
        ...
        if (
          payload &&
          (payload.type === "external-entity" || payload.type === "external-subject")
        ) {
        ...
        if (
          ...
          [block.items[sourceItemIndex], block.items[targetItemIndex]] = 
            [block.items[targetItemIndex], block.items[sourceItemIndex]];
    ...
    addEntityToExpression(entityId, entityType, element) {
      ...
      if (insertIndex === this.currentExpression.length) {
        ...
        this.currentExpression.push({
          type: "andBlock",
          items: [newItem],
          operators: [], // Add operators array to store operator types between items
        });
      } else {
